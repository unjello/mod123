...
const debug = require('debug')('libmod123')
...
module.exports = function(buffer, options) {
  ...
  readable.channels = channels;
  readable.sampleRate = samplerate;
  readable._read = () => {
    debug('_read max', maxFramesPerChunk*bytesPerFrame)
    if(destroyed) {
      debug('destroyed. pushing null.')
      readable.push(null);
      return;
    }
    const data = readModule(mod, buf, samplerate, channels, maxFramesPerChunk, bytesPerFrame)
    if (buf == null) {
      debug('buffer null. destroying module.')
...
function readModule(mod_ptr, buf_ptr, samplerate, channels, maxFramesPerChunk, bytesPerFrame) {
  ...
  debug('read', frames, 'frames', bytesPerFrame*frames, 'bytes')
